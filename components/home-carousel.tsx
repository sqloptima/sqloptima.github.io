...
const slides = [
  ...
  { eyebrow: "Product", title: "SQL Monitoring", lede: "Health and performance telemetry for SQL Server and PostgreSQL.", highlights: ["Live health and wait signals", "Rule-based checks", "Actionable query triage"], image: "/images/monitoring/sqlserver-dashboard.png", href: "/products/sql-monitoring/", cta: "Explore SQL Monitoring" },
  { eyebrow: "Product", title: "Schema Compare", lede: "Diff procedures, indexes, and tables across environments.", highlights: ["Windows GUI", "Cross-platform CLI", "Review drift before release"], image: "/images/products/sql-optima-schema-compare-hero.png", href: "/products/schema-compare/", cta: "Explore Schema Compare" },
  { eyebrow: "Product", title: "Migration Toolkit", lede: "Move from SQL Server to PostgreSQL on a plan you rehearsed.", highlights: ["Procedure conversion", "Function conversion", "Cutover guidance"], image: "/images/products/sql-optima-migration-hero.png", href: "/products/migration-toolkit/", cta: "Explore Migration Toolkit" },
  { eyebrow: "Product", title: "Assessment Generator", lede: "Turn SQL Server and PostgreSQL evidence into clear health reports.", highlights: ["Quick, Standard, and Deep", "Severity-scored findings", "HTML and PDF reports"], image: "/images/products/dba-handbook-hero.png", href: "/products/assessment-generator/", cta: "Explore Assessment Generator" },
  { eyebrow: "Product", title: "Backup Manager", lede: "Open-source SQL Server backup operations for Windows.", highlights: ["FULL / DIFF / LOG chains", "Restore confidence", "Web operations console"], image: "/images/products/sqloptima-backup-pro-hero.png", href: "/products/backup-manager/", cta: "Explore Backup Manager" },
  { eyebrow: "Product", title: "DBA Handbook", lede: "Checklists, scripts, and reports for live SQL Server and PostgreSQL.", highlights: ["Production playbooks", "Portable guidance", "Ready-to-run scripts"], image: "/images/handbook/knowledgebase-hero.png", href: "/products/dba-handbook/", cta: "Explore DBA Handbook" },
  { eyebrow: "Product", title: "HA Cluster Lab", lede: "Practice failover with PostgreSQL and SQL Server side by side.", highlights: ["Patroni PostgreSQL", "SQL Server cluster", "Repeatable failover traffic"], image: "/images/monitoring/postgres-dashboard.png", href: "/products/ha-cluster-lab/", cta: "Explore HA Cluster Lab" },
] as const;
...
export function HomeCarousel() {
  ...
  return <section className="source-hero" aria-roledescription="carousel" aria-label="sqloptima products" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
    ...
    <div className="source-hero-copy"><span className="source-pill">{slide.eyebrow}</span><h1 id="home-title">{slide.title}</h1><p>{slide.lede}</p><ul>{slide.highlights.map((item)=><li key={item}>{item}</li>)}</ul><a href={slide.href.startsWith("/") ? withBasePath(slide.href) : slide.href}>{slide.cta} <span aria-hidden>→</span></a></div>
